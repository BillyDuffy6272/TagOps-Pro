...
| Organisation for Conversion events; link a container to a Google Ads conversion ID and generate ready-to-paste tracking code | **Met, plus a partially-verified extension** | Removed 28/08/2026 (ADR-0038), re-added 17/09/2026 (ADR-0041) — the manual entry/snippet/local-verification flow is back, unchanged from what was previously built and tested. On top of it, a new real Google Ads OAuth connection (`GoogleAdsConnectionPanel`, two Edge Functions) was built to pull *live* conversion data, which is what this requirement's own MVP line always actually implied. The OAuth-connect half is verified (a real Google account can complete it today); the live Ads-data pull has never run against a real endpoint — no developer token exists (ADR-0037) — and is explicitly flagged as such rather than claimed as done. See `decision-log.md` ADR-0041 |
...
- **RLS-first design held up under real audit pressure.** RLS-as-the-authorisation-mechanism was decided in Week 2, before any feature code existed (`decision-log.md` ADR-0003) — a deliberate architectural call, not something adopted mid-project. All twelve domain tables have RLS enabled with no blanket-permissive policy (one of them, `google_ads_connections`, goes further and grants no client policies at all — ADR-0041); when two real bugs were found, they were found by policy-by-policy review, not by a live exploit — the database refused to let a corrupted UI state cause real damage in the meantime.
...
- **A working, if small, automated test suite.** 44 unit tests across three files: 29 in `tests/unit/simulator.test.ts` (Preview mode's dataLayer/tag-firing simulator), 7 in `tests/unit/snippets.test.ts` (the conversion-event tracking-code generator, incl. two adversarial injection tests — removed with the feature in ADR-0038, restored unchanged in ADR-0041), and 8 new in `tests/unit/googleAds.test.ts` (the Google Ads report path's pure functions only — the live token-exchange/API call itself is untested, deliberately, per ADR-0041). The pattern `snippets.test.ts` demonstrated — adding a regression test *because* something already went wrong once — is worth keeping in mind for whatever's tested next.
...
See `08-test-plan.md` for the full breakdown. Unit tests exist and pass (44/44, up from the original 29) for dataLayer-touching logic and the restored/new conversion-tracking and Google Ads code, per `CLAUDE.md`'s requirement. Both the RLS integration suite and the Playwright smoke suite named in the mandated repo layout now exist (ADR-0040) and have **both** actually been run (the RLS suite as of ADR-0042, once Docker became available): the smoke suite passes 3/3 (scoped to the signed-out flow); the RLS suite passes 11/12, with the one known failure being a real, documented, deliberately-unfixed gap (the `tags`/`triggers`/`variables` container/organisation-consistency issue — `05-security-review.md`), not an unrun or type-checked-only claim. `npm run lint`/`typecheck`/`test`/`test:integration`/`build` all passing against the current migration set is a materially stronger claim now than it was when the integration suite's status was still "written, never executed."

## Future improvements

In priority order if development continued: **fix the `tags`/`triggers`/`variables` container/organisation-consistency gap** the now-run RLS suite surfaced (ADR-0042) if those tables are ever put to real use; **verify `supabase/functions/google-ads-report` against a real Google Ads endpoint** once a developer token exists; extend the Playwright suite past the signed-out flow, most plausibly via a captured `storageState` from a real sign-in; TanStack Query to replace the currently-duplicated fetch-orchestration logic across four views; the missing `@/` import alias; a Content-Security-Policy; rate limiting on invite-code redemption; and closing the two deliberately-deferred accessibility gaps.
