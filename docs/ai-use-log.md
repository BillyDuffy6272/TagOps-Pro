...
### Re-added conversion tracking, then built a real (deliberately unverified) Google Ads OAuth integration on top

**Date:** 17/09/2026

**Prompt:** "Re-add conversions." Before writing anything, surfaced that this was a deliberate removal (ADR-0038) and asked which version was actually wanted — the answer, across two follow-up questions, was the full live Google Ads API integration ADR-0037 researched and declined to build, built anyway without a real developer token or sandbox account ("build it blind"), an explicit, informed choice to repeat the exact risk ADR-0037 flagged.

**Response summary:** Researched before proposing anything: found `src/pages/Login.tsx` already requests the `tagmanager.readonly` scope on every sign-in, which is exactly the call ADR-0037 warned not to extend with the Google-restricted `adwords` scope (would risk breaking login for any real user while the app's OAuth consent screen is in Testing status). Wrote a plan (via plan mode) restoring the local conversion-tracking layer first — recovered verbatim from git history at the commit right before ADR-0038's removal, already carrying every prior fix — then adding a genuinely separate "Connect Google Ads" flow that never touches `Login.tsx`: a new panel makes its own `signInWithOAuth` call with both scopes together, only when explicitly clicked by an owner/admin, with the resulting refresh token captured once in `AppShell.tsx` (the one place mounted for the whole app) and handed to a new Edge Function rather than ever sitting in a plain database column. Used Supabase Vault for the token (`google_ads_connections` table with zero RLS policies for `anon`/`authenticated` at all — invisible from the client regardless of role), following through on a "stored encrypted via Supabase Vault" line that had sat unactioned in `04-data-model.md`'s roadmap section since before this session started. Wrote the first two Edge Functions this repo has ever had. Because no real Google Ads developer token exists, the report function checks for the secret before ever calling Google and returns a clean `developer_token_not_configured` response instead of a raw failure — meaning the OAuth-connect half of this work is actually verifiable today (a real Google account can complete the consent screen right now), while the live Ads-data pull has never been run against a real endpoint, called out plainly rather than blurred with the parts that were tested. Logged as `decision-log.md` ADR-0041. Full detail (file-by-file) is in that ADR rather than duplicated here.

**What you did with it:** Accepted, including the explicit choice to ship the unverified report path rather than stopping at the local-only restore.

**Why:** Two decision points here were genuinely the user's to make, not defaults to assume — which version of "conversions" to rebuild, and whether to accept shipping code that breaks this project's own "verify against a live endpoint" precedent — so both were surfaced as direct questions before any code was written, rather than picking the safer-looking option unasked. Once the direction was confirmed, the actual engineering judgement (keeping the new OAuth scope off the login path, using Vault instead of a plain column for the refresh token, returning a clean blocked-state response instead of a raw error when the developer token is missing) was mine to make and defend, same as every other ADR in this log.

---

### Tested ADR-0041 for real — found and fixed a live PII-disclosure bug plus a gap in my own new code

**Date:** 18/09/2026

**Prompt:** "Just test everything added and make sure it safe for deployment and just generally works."

**Response summary:** Docker turned out to be available this time (it wasn't for ADR-0040), so rather than re-reading SQL and reasoning about it, actually ran `supabase start` and the previously-never-executed RLS integration suite against a real local Postgres. Its very first test failed — investigated rather than assumed it was a fixture bug, and confirmed via direct SQL (impersonating both roles under `set role`) that `conversion_events`' INSERT/UPDATE policies never check that `container_id` actually belongs to the `organisation_id` being written, though also confirmed this doesn't leak data cross-org (the victim org's own RLS still hides the row). Fixed it for `conversion_events` specifically, via a new migration adding a `container_belongs_to_org()` check. While separately verifying the new Google Ads Vault functions actually worked (not just that the migration applied without error), found a real bug in today's own earlier work: `get_google_ads_refresh_token` — meant to be `service_role`-only — was callable by any authenticated user and returned the raw stored token for any organisation, because Supabase grants EXECUTE on new functions directly to `anon`/`authenticated` (not just `PUBLIC`), so the `revoke ... from public` I'd written was a silent no-op. The same check against every other `SECURITY DEFINER` function in the schema surfaced a second, **pre-existing, already-live** instance of the identical mistake: `find_user_by_email` (three weeks old, unrelated to today's work) has no internal auth check at all, so a fully anonymous caller — just the public anon key, no login — could already look up any real user's id/name/avatar/email. Fixed both (the Google Ads functions in place, since never applied anywhere; `find_user_by_email` and the three invite-code functions via a new migration, since that file likely already shipped to the real linked project). Added regression tests for both findings to `tests/integration/rls.test.ts` plus a new `impersonateAnon()` helper. Then verified the two new Edge Functions end-to-end over real HTTP against a real GoTrue-issued session (created via the admin API) rather than only unit-testing their pure helper functions — confirmed the auth/membership/role gates and the `developer_token_not_configured` fallback all behave exactly as designed. Logged as `decision-log.md` ADR-0042.

**What you did with it:** Accepted, including the `find_user_by_email` fix being pushed to the real project sooner than the rest of ADR-0041.

**Why:** "Make sure it's safe for deployment" only means something if it's actually checked against a real database, not re-derived from reading policy text — this session had the one thing (Docker) the previous testing session didn't, and using it turned a "should be fine" into either "confirmed fine" or, twice, "actually not fine, here's why and here's the fix." The `find_user_by_email` finding specifically wasn't something I went looking for — it came from checking whether my own new code's mistake was unique to today's work or systemic, which it was.
