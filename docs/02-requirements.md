...
> Note (added 17/09/2026): Conversion-event tracking was re-added (`decision-log.md` ADR-0041) — the MUST HAVE / WOULD BE NICE lines above are accurate again, not just historical. The "Out of scope" bullet directly above is now only partially true: a real Google Ads OAuth connection was built (a new "Connect Google Ads" flow, kept deliberately separate from the login OAuth call), but the developer-token approval process that bullet describes is still exactly as blocked as it was — no token exists, so the live conversion-data pull has never been run against a real Google Ads endpoint. See ADR-0041 for the full detail on what's verified versus not.