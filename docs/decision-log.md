...
## ADR-0040 — Built the RLS integration suite and the Playwright smoke suite; only one of them is actually verified

- **Date:** 2026-08-28
- **Status:** Accepted
- **Context:** Asked to explain what an RLS integration suite, a Playwright suite, and UAT actually are, then asked to implement everything that didn't require the user directly — explicitly excluding UAT (needs a real outside tester) but including the other two, both already named as gaps in `08-test-plan.md`'s mandated repo layout and never built.
- **Decision:**
  1. **`tests/integration/rls.test.ts`** (+ `tests/integration/helpers.ts`) — 7 tests impersonating two different authenticated users in two different organisations via `set_config('request.jwt.claims', ...)` + `SET LOCAL ROLE authenticated` inside a rolled-back transaction, the exact mechanism PostgREST uses per request. Every test targets a real, already-documented threat rather than generic coverage: cross-org INSERT/SELECT on `tags` (with same-org positive controls), the specific ADR-0029/ADR-0033 `organisation_members` cross-tenant takeover, the `role <> 'owner'` UPDATE boundary, and an expired-membership write attempt. Wrote the SQL directly against the real table/policy definitions in `supabase/migrations/` (read fresh, not from memory) rather than guessing, and caught two real bugs in the *test* code itself this way before ever running anything: the `organisation_members.expires_at > joined_at` CHECK constraint would have rejected an "already expired" fixture outright (fixed by backdating `joined_at` too), and an UPDATE whose row passes `USING` but fails `WITH CHECK` throws in Postgres rather than silently affecting zero rows (fixed the test's own assertion, not the policy). Added `pg`/`@types/pg` as devDependencies for this — flagged rather than installed silently, per `CLAUDE.md`'s "ask before installing a dependency."
  2. **`tests/smoke/landing-login.spec.ts`** + `playwright.config.ts` — 3 Playwright tests against a real production build (`npm run build` + `vite preview`), actually run and passing, not just written. Deliberately scoped to the signed-out Landing → Login flow only: every other view needs a real Google-authenticated Supabase session, and a hand-crafted JWT would pass the client's own login check but fail signature validation on every real query — proving the gate can be bypassed, not that anything past it works. Added `@playwright/test` as a devDependency and installed the Chromium binary.
  3. **Test-runner scoping.** `npm run test` now explicitly targets `tests/unit` (was bare `vitest run`, which would have silently picked up the new integration/smoke files too). New `npm run test:integration` (`vitest run tests/integration`) and `npm run test:smoke` (`playwright test`). `vite.config.ts` gained a `test.exclude` for `tests/smoke/**` as defense-in-depth against someone running bare `vitest`, needing `/// <reference types="vitest/config" />` to type-check (Vite's own config type doesn't know about `test` without it — this broke `npm run build` once and was caught before commit). `.gitignore` gained Playwright's `test-results`/`playwright-report` output dirs.
- **Consequences:** This is not "both gaps closed" — it's one gap closed (smoke, verified) and one gap **converted from "doesn't exist" to "exists but unverified"** (RLS integration), which is real progress but a materially different claim, stated as such in `08-test-plan.md` and `07-evaluation.md` rather than blurred together. The RLS suite could not be run in the environment that wrote it — no Docker, so no `supabase start`, so no local Postgres to connect to — meaning its SQL correctness rests on careful reading of the real schema/policies rather than a passing test run. Whoever picks this up next should run `supabase start && npm run test:integration` before trusting it, and treat any failure as possibly the test's own fixtures rather than a real policy bug until checked. `npm run lint`/`typecheck`/`test` (unit) all still pass; `npm run test:smoke` passes 3/3, actually executed.
