...
- **Date:** 2026-08-25
- **Status:** Accepted
- **Context:** The app was dark-only: `index.css`'s `@theme` block defined canvas/surface/border/text/accent/status colors as fixed hex values, with no light equivalents. Asked for a Light/Dark/System switch in Settings. Scoping check before writing any code: a correct light theme needed more than new color values, because ~43 hover/active/ring states across 18 components were hardcoded as literal `bg-white/N`, `border-white/N`, `ring-white/N` (plus a handful of inline `rgba(255,255,255,…)` shadows and the scrollbar thumb) — all of which *lighten* a surface, which is backwards on a light background. Left alone, "Light mode" would have shipped with invisible or inverted hover states throughout the app.
- **Decision:** Token-level theming, not a parallel light stylesheet: kept every existing CSS custom property name (`--color-canvas`, `--color-surface-*`, `--color-text-*`, `--color-accent`, `--color-success/warning/danger`) and added light-mode values under `@media (prefers-color-scheme: light) { :root:not([data-theme]) { … } }` for system-light, plus explicit `:root[data-theme="light"]` and `:root[data-theme="dark"]` blocks so an explicit choice always wins. Added one new token, `--color-overlay` (white in dark, black in light), registered in `@theme` so Tailwind generates `bg-overlay/N` / `border-overlay/N` / `ring-overlay/N` utilities exactly like the `white/N` ones they replace — then mechanically renamed every hardcoded `white` overlay utility across the 18 affected files to `overlay`, and converted the handful of hand-written `rgba(255,255,255,…)` shadows (Sidebar's active-state inset highlights, FilterTabs' active-tab highlight, the scrollbar thumb) to `color-mix(in srgb, var(--color-overlay) N%, transparent)`. Light-mode `accent`/`success`/`warning`/`danger` were darkened from their dark-mode values (e.g. accent `#8b8ef7` → `#5b52e0`) since several are used as button-background-with-light-text, which needs a different contrast target than text-on-dark-background. A new `ThemeContext` (`src/lib/ThemeContext.tsx`) holds the `light | dark | system` choice, persists it to `localStorage`, and applies it as a `data-theme` attribute on `<html>` — applied synchronously in `main.tsx` before React's first render to avoid a flash of the wrong theme, with `ThemeProvider` re-deriving the same value on mount. A three-way segmented control was added to Settings, in a new "Appearance" section under Profile.
- **Consequences:** Every existing component that already used the semantic tokens (`bg-canvas`, `text-text-primary`, etc.) got light-mode support for free — no per-component palette work needed beyond the overlay rename. The category-badge dot colors (`bg-sky-400`, `bg-amber-400`, etc. in `CategoryBadge.tsx`) were deliberately left as fixed Tailwind palette colors rather than tokenized, since 400-weight swatches read clearly on either background without adjustment. Verified visually in both themes via a temporary preview bypass in `App.tsx` (Playwright screenshots of the shell and Settings in light and dark), fully reverted afterward — `git diff` on `App.tsx` is empty. Any future component reaching for `bg-white/N`, `border-white/N`, or `ring-white/N` directly (instead of `overlay`) will silently break in light mode — worth a lint rule or at least a callout in `CLAUDE.md`'s coding conventions if it recurs.
...
- **Context:** Building ADR-0022's Settings page surfaced a real, previously-invisible gap: `Dashboard.tsx` never checked whether a signed-in user actually has an `organisation_members` row. `handle_new_user()` mirrors every Google sign-in into `public.users`, but nothing creates an organisation or a membership for them — the only reason this hadn't been hit yet was that the one real test account was manually granted access to the seeded demo org. Any other real sign-in would reach Conversions or Settings and hit "No organisation membership found" with no way out. The user asked directly for the ability to create or join an organisation, linked from Settings. "Create" was already schema-supported (`authenticated users can create organisations` policy + the `auto_owner_membership()` trigger); "join" had no mechanism at all. Asked one scoping question — a self-service shareable invite code vs. no self-serve join at all (admin adds you by email, already built in ADR-0022) — and the invite-code option was chosen.
- **Decision:** New migration `20260818000000_organisation_invite_codes.sql` adds `organisations.invite_code`, but deliberately **not** as a plain readable column: RLS is row-level only, and the existing "members can select organisations" policy would let even a viewer read (and redistribute) it if it were just another column. Instead, column-level `SELECT`/`UPDATE` on `invite_code` is revoked from `authenticated` entirely, and three `SECURITY DEFINER` functions are the only way to touch it, each re-checking the caller's role inside the function body rather than leaning on table RLS: `get_invite_code`/`regenerate_invite_code` (owner/admin only; the code is generated lazily on first request, not at org creation, since most orgs may never need to invite anyone) and `redeem_invite_code` (any authenticated user; looks up the org by code, inserts an `organisation_members` row for the caller as `editor` with no expiry if they're not already a member, and is a no-op — not an error — if they are). Because column-level SELECT is revoked, `getOrganisation()`/`updateOrganisationName()` in ADR-0022's Settings feature had to move off `select('*')`/`select()` onto an explicit column list (`OrganisationSummary`, an `Omit<Organisation, 'invite_code'>` type) — a bare `*` would now fail outright for every caller, not just non-admins. New `src/features/organisation/` feature: `OrganisationOnboarding` (a full-screen, pre-`AppShell` gate with Create/Join tabs, styled like `Login.tsx` since there's no sidebar or org context yet to build a normal view around) and `OrganisationView` (a normal in-app page — shows the current org and, for owner/admin, the invite code with copy/regenerate — reached via a new "Invite teammates" button in Settings, the same way Settings itself is reached from the sidebar's profile row rather than the primary `NAV_ITEMS` list). `Dashboard.tsx` now checks membership via a new non-throwing `checkMyMembership()` (added alongside the existing throwing `getMyMembership()` in `lib/organisation.ts`, which now calls it) and renders `OrganisationOnboarding` in place of `AppShell` when a session has zero memberships, distinguishing that genuine empty state from a real fetch error (which shows a retry screen instead) so a transient Supabase blip can't be misread as "you have no organisation."
- **Consequences:** A user can now only ever be a member of one organisation through this UI — creating a second one from `OrganisationView` was deliberately left out, since `getMyMembership()`'s `.limit(1)` has no concept of an "active" org to switch between and building that switcher is out of scope here; multi-org support stays a schema-level possibility, not a real feature yet. **The new migration has not been run against the live Supabase project yet** — until it is, both the onboarding gate's "Join with code" tab and Settings' new "Invite teammates" button will fail (missing RPCs). This is the second migration in a row to touch `public.users`/`organisations` access at the column/function level rather than through a plain RLS row policy — worth calling out together with ADR-0022's `users` SELECT widening in `docs/05-security-review.md` once that file starts, since both are deliberate, scoped trade-offs on the same two tables, not incidental.
...
- **Date:** 2026-08-09
- **Status:** Accepted — **migration not yet applied to the live Supabase project** (see Consequences)
- **Context:** The user asked for a Settings/Profile page, "something that would be needed." `docs/02-requirements.md` already lists "Add people to the Account", "Remove people from the Account", and "Add an expiry date for people" as MVP must-haves, and the schema from ADR-0004/0005 (`organisation_members` with `role` + `expires_at`, full RLS) has existed since Week 5–6 with no UI ever reading or writing it. Two RLS gaps stood in the way of a real team list: (1) `public.users` only allows selecting your own row, so a join from `organisation_members` to `users` would return every teammate's name/email/avatar as null; (2) adding someone by email requires looking up a user who is by definition *not yet* a teammate, which gap #1's fix doesn't cover.
- **Decision:** New migration `20260809000000_settings_team_management.sql`: (a) an additive `users` SELECT policy — "anyone who shares an organisation with you can see your basic profile" — scoped to shared membership, not a global directory; (b) a `find_user_by_email(lookup_email)` SECURITY DEFINER function, grant restricted to `authenticated`, returning only `id`/`display_name`/`avatar_url`/`email` for an exact match, so "add by email" doesn't require widening the SELECT policy into a full user-enumeration surface. New feature `src/features/settings/` (types, `api/settings.ts`, `SettingsView`/`MemberRow`/`AddMemberModal`) covers: a read-only profile card sourced from the Google OAuth session (not `public.users` — that table intentionally has no UPDATE policy, so editing it in-app would just be overwritten on next sign-in anyway); an organisation-name field editable only by the owner (matches the existing "owners can update organisations" policy); and a team list with per-row role change, expiry date, and remove, gated in the UI by the caller's own role (owner/admin) — RLS is the actual enforcement, the UI just reflects it. "Owner" is deliberately not an assignable role from this UI (ownership transfer is a separate, more sensitive operation) and the owner row is display-only. `getCurrentOrganisationId`/`generateDisplayId` were promoted from `conversions/api/conversions.ts` (their only prior caller) to a new `src/lib/organisation.ts`, since Settings needed the same "which org am I in" lookup and duplicating it a second time was the wrong call — extracted as `getMyMembership()` (now returning role too) and reused by `conversions.ts` with no behaviour change there. Reached the sidebar via the existing profile row (avatar + name) in the footer, not a new item in the primary `NAV_ITEMS` "Workspace" list, since account-level settings is a different category from the GTM-entity views.
- **Consequences:** "Add a member" only works for someone who has already signed in to TagOps-Pro at least once (their row must exist in `public.users`) — inviting someone who has *never* signed up would need a separate pending-invite-plus-email-sending system (a Supabase Edge Function), which is a real follow-up, not something folded into this session. **The new migration has not been run against the live Supabase project yet** — until it is, the Settings page's team list and "Add member" will fail with a permissions/RPC-not-found error; this needs the same SQL-editor step as every prior migration before it can be exercised end-to-end. `src/types/supabase.ts` gained hand-authored `users` and `organisations` table types plus a `find_user_by_email` `Functions` entry (same no-CLI caveat as every prior hand-authored addition — see ADR-0012/0013). Worth a `docs/05-security-review.md` entry once that folio file starts (Week 6–7 per the timeline): the widened `users` SELECT policy is the one new piece of cross-user data exposure in the schema and should be named explicitly in the threat model, not just in this ADR.
